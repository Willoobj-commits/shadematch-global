...
- Workspace root: `D:\edic-bandbook`.
...
- Extracted app package to `D:\edic-bandbook\shade_match_global_flutter` - validated 2026-09-01 - `pubspec.yaml`, `lib\main.dart`, and `assets\data\catalog_v1.json` exist.
...
- Built Android release APK - validated 2026-09-01 - artifact at `D:\edic-bandbook\shade_match_global_flutter\build\app\outputs\flutter-apk\app-release.apk` (51,739,759 bytes).
...
- Built signed Android App Bundle - validated 2026-09-01 - artifact at `D:\edic-bandbook\shade_match_global_flutter\build\app\outputs\bundle\release\app-release.aab` (50,926,769 bytes); `jarsigner -verify` reports `jar verified`.
...
1. Upload `D:\edic-bandbook\shade_match_global_flutter\build\app\outputs\bundle\release\app-release.aab` to Play Console internal testing - acceptance criteria: Play accepts package `com.shadematchglobal.shade_match_global`, version `0.1.0+1`, signed with the generated upload key.
2. Save a private backup of `D:\edic-bandbook\shade_match_global_flutter\android\app\upload-keystore.jks` and `D:\edic-bandbook\shade_match_global_flutter\android\key.properties` outside the repository - acceptance criteria: upload key can be recovered for future version uploads.
3. Optional: install or distribute `D:\edic-bandbook\shade_match_global_flutter\build\app\outputs\flutter-apk\app-release.apk` to an Android device for manual QA - acceptance criteria: app launches, bottom navigation works, catalog search opens shade details, and saved shades persist.
4. Optional: build iOS on macOS/Xcode - files: `D:\edic-bandbook\shade_match_global_flutter\ios` - acceptance criteria: iOS archive/testflight build succeeds in an Apple toolchain environment.
